import { useRef, useState } from "react"; type Msg = { role: "user" | "assistant"; content: string }; export default function Home() { const [msgs, setMsgs] = useState<Msg[]>([ { role: "assistant", content: "Welcome to Althea Chat. What would help right now — reflection, focus, or a gentle check‑in?" } ]); const [input, setInput] = useState(""); const [sending, setSending] = useState(false); const outRef = useRef(null); async function send() { const text = input.trim(); if (!text || sending) return; const history = [...msgs, { role: "user", content: text }]; setMsgs(history); setInput(""); setSending(true);
